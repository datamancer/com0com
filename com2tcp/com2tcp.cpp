...
#define P_TCP 0
#define P_UDP 1
static char pProtoNumber = P_TCP;
static const char *pProtoName[] = { "tcp", "udp" };
...
///TODO:  fix the damn buffer overflow bug.
  ...
  while (!stop) {
    ...
    if (pProtoNumber == P_UDP && !waitingRecv && !protocol.isWriteFull()) {
      if (!ReadFile((HANDLE)hSockListen, cbufRecv, sizeof(cbufRecv), &not_used, &overlaps[EVENT_RECEIVED])) {
        if (::GetLastError() != ERROR_IO_PENDING) {
          TraceLastError("InOut(): ReadFile(hSockListen)");
          break;
        }
      }
      waitingRecv = TRUE;
	...
	const BYTE *pAwakSeq)
{
	...
	SOCKET hSockListen = Socket(pIF, pLocalPort);
